...
export function CoursePaymentForm({ course }: CoursePaymentFormProps) {
    ...
    const removeImage = () => {
        setData('identity_image', null);
        setImagePreview(null);
        // Reset file input
        const fileInput = document.getElementById('identity_image') as HTMLInputElement;
        if (fileInput) {
            fileInput.value = '';
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('courses.payment.store'), {
            forceFormData: true,
            onSuccess: () => {
                reset();
                setImagePreview(null);
                ...
                <Card className="border-border bg-card">
                    ...
                    <div className="space-y-2">
                        ...
                        {errors.identity_image && (
                            <p className="text-sm text-destructive">{errors.identity_image}</p>
                        )}
                    </div>
