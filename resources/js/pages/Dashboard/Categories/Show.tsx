...
interface Category {
  id: number;
  name: string; // localized automatically by backend
  slug: string;
  courses_count: number;
  courses: Course[];
  translations?: TranslationRow[]; // provided by controller for admin visibility
}
...
export default function ShowCategory({ category }: Props) {
  ...
  const hasTranslations =
    Array.isArray(category.translations) && category.translations.length > 0;

  const arTranslations = (category.translations || []).filter(
    (t) => t.locale === 'ar'
  );
      ...
      <div className="space-y-6">
                  ...
                  <AlertDialogDescription>
                    ...
                    {category.courses_count > 0 && (
                      <span className="block mt-2 font-medium text-destructive">
                        This category has {category.courses_count} associated
                        course{category.courses_count !== 1 ? 's' : ''}.
                      </span>
                    )}
                  </AlertDialogDescription>
                </AlertDialogHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Translations section for admins */}
            {hasTranslations && (
              <div className="pt-4 mt-2 border-t">
                ...
                {/* Arabic translations box (highlighted) */}
                {arTranslations.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs text-muted-foreground mb-1">Arabic</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {arTranslations.map((row, idx) => (
                        <div key={`ar-${idx}`} className="p-3 rounded-md border bg-muted/30">
                ...
                {/* Any other locale (future-proof) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {(category.translations || [])
                    .filter((t) => t.locale !== 'ar')
                    .map((row, idx) => (
                      <div key={`tr-${idx}`} className="p-3 rounded-md border bg-muted/30">
                        <div className="text-xs text-muted-foreground mb-1">
